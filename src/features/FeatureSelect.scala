  ...
  * VectorSlicer算法介绍：
    ...
    VectorSlicer是一个转换器输入特征向量，输出原始特征向量子集。VectorSlicer接收带有特定索引的向量列，通过对这些索引的值进行筛选得到新的向量集。可接受如下两种索引：
      ...
      2、字符串索引---代表向量中特征的名字，这要求向量列有AttributeGroup，因为这根据Attribute来匹配名字字段
        指定整数或者字符串类型都是可以的。另外，同时使用整数索引和字符串名字也是可以的。同时注意，至少选择一个特征，不能重复选择同一特征（整数索引和名字索引对应的特征不能叠）。
...
userFeatures是一个向量列包含3个用户特征。假设userFeatures的第一列全为0，我们希望删除它并且只选择后两项。
我们可以通过索引setIndices(1,2)来选择后两项并产生一个新的features列：
...
object FeatureSelect {
  ...
  def example1(spark: SparkSession): Unit ={
    ...
    val attrs: Array[Attribute] = Array("f1", "f2", "f3").map(defaultAttr.withName)
